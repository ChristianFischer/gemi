...
/// output channels.
pub struct Apu {
    /// Remaining time until the next audio sample has to be generated.
...
impl Apu {
    ...
    pub fn update(&mut self, cycles: Clock) {
        ...
        self.generate_audio_samples(cycles);
    }
    ...
    fn update_channels(&mut self, cycles: Clock) {
        self.ch1.update(cycles);
        self.ch2.update(cycles);
        self.ch3.update(cycles);
        self.ch4.update(cycles);
    }


    /// Generates audio samples and stores them into the audio buffer.
    /// This function measures the time passed since the last audio sample generated
    /// to generate samples depending on the output buffers sample rate.
    fn generate_audio_samples(&mut self, cycles: Clock) {
        let cycles_per_sample = self.output_buffer.get_cycles_per_sample();
        self.next_sample_time += cycles;

        while self.next_sample_time >= cycles_per_sample {
            self.next_sample_time -= cycles_per_sample;
