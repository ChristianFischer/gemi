...
pub type SamplesSender = Sender<SampleBuffer>;

/// Receiver part of the channel to transfer audio samples from the APU to the consumer.
pub type SamplesReceiver = Receiver<SampleBuffer>;


/// Stores the configuration to initialize the autio generation.
    ...
    fn finish_sample(&mut self) {
        ...
        if self.buffer_insert_pos >= SAMPLE_BUFFER_SIZE {
            ...
            if let Some(sender) = &self.sender {
                let result = sender.send(self.buffer);
