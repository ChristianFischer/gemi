...
impl WaveGenerator {
    ...
    pub fn get_wave_ram_access(&self, ec: &impl EmulatorClient, requested_address: u16) -> Option<u8> {
    ...
    pub fn read_wave_ram(&self, ec: &impl EmulatorClient, requested_address: u16) -> u8 {
    ...
    /// This takes into account when the access is restricted when the channel is enabled.
    pub fn write_wave_ram(&mut self, ec: &impl EmulatorClient, requested_address: u16, value: u8) {
        if let Some(address) = self.get_wave_ram_access(ec, requested_address) {
