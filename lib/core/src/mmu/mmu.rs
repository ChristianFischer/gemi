...
impl Mmu {
    ...
    pub fn read_u8(&self, ec: &impl EmulatorContext, address: u16) -> u8 {
        self.internal.read(ec, address)
    }


    /// Reads two bytes into a 16 bit integer from the memory bus on a given address.
    pub fn read_u16(&self, ec: &impl EmulatorContext, address: u16) -> u16 {
    ...
    pub fn write_u8(&mut self, ec: &mut impl EmulatorContextMut, address: u16, value: u8) {
        self.internal.write(ec, address, value);
    }


    /// Writes two bytes from a 16 bit integer to the memory bus on a given address.
    pub fn write_u16(&mut self, ec: &mut impl EmulatorContextMut, address: u16, value: u16) {
        let (h, l) = to_u8(value);
...
impl MmuInternal {
    ...
    fn handle_dma_transfer(&mut self, ec: &impl EmulatorContext, cycles: Clock) {
        match self.dma {
