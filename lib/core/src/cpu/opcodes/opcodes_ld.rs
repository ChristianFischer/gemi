...
fn ld_r8_u8(dev: &mut EmulatorDevice, dst: RegisterR8) {
    let value = dev.cpu.fetch_u8();
    dev.cpu.set_r8(dst, value);
}

/// Loads the content of a 8bit register into the device memory.
fn ld_addr_r8(dev: &mut EmulatorDevice, dst_address: u16, src: RegisterR8) {
    let value = dev.cpu.get_r8(src);
    dev.get_mmu_mut().write_u8(dst_address, value);
}

/// Writes a 8bit value to a given address in the device memory.
fn ld_addr_u8(dev: &mut EmulatorDevice, dst_address: u16, value: u8) {
    dev.get_mmu_mut().write_u8(dst_address, value);
}

/// Writes a 8bit value to a given address in the device memory.
fn ld_addr_u16(dev: &mut EmulatorDevice, dst_address: u16, value: u16) {
    dev.get_mmu_mut().write_u16(dst_address, value);
}

/// Loads the value on a given address into a 8bit register.
fn ld_r8_addr(dev: &mut EmulatorDevice, dst: RegisterR8, src_address: u16) {
    let value = dev.get_mmu().read_u8(src_address);
    dev.cpu.set_r8(dst, value);
}

/// Loads the content of a 8bit register into the address stored in the target R16 register.
fn ld_r16ptr_r8(dev: &mut EmulatorDevice, dst: RegisterR16, src: RegisterR8) {
    let address = dev.cpu.get_r16(dst);
    ld_addr_r8(dev, address, src);
}

/// Loads the content of a 8bit register into a constant address of the device memory.
fn ld_u16ptr_r8(dev: &mut EmulatorDevice, src: RegisterR8) {
...
fn ld_r16ptr_u8(dev: &mut EmulatorDevice, dst: RegisterR16) {
    let address = dev.cpu.get_r16(dst);
    let value   = dev.cpu.fetch_u8();
    ld_addr_u8(dev, address, value);
}

/// Loads a 16bit value into a constant address of the device memory.
fn ld_u16ptr_u16v(dev: &mut EmulatorDevice, value: u16) {
    let address = dev.cpu.fetch_u16();
    ld_addr_u16(dev, address, value);
}

/// Loads the value at the address stored in a 16bit register
/// into a 8bit register.
fn ld_r8_r16ptr(dev: &mut EmulatorDevice, dst: RegisterR8, src: RegisterR16) {
    let address = dev.cpu.get_r16(src);
    ld_r8_addr(dev, dst, address);
}

/// Loads the value at the address stored in a 16bit constant
/// into a 8bit register.
fn ld_r8_u16ptr(dev: &mut EmulatorDevice, dst: RegisterR8) {
    let address = dev.cpu.fetch_u16();
    ld_r8_addr(dev, dst, address);
}

/// Loads a constant 16bit value from the current instruction pointer into a 16bit register.
fn ld_r16_u16(dev: &mut EmulatorDevice, dst: RegisterR16) {
    let value = dev.cpu.fetch_u16();
    dev.cpu.set_r16(dst, value);
}

/// Loads the value of a 8bit register into the device memory at the address (0xff00 + u8).
fn ldh_u8_r8(dev: &mut EmulatorDevice, src: RegisterR8) {
    let value     = dev.cpu.get_r8(src);
    let address_h = dev.cpu.fetch_u8();
    let address   = 0xff00 | (address_h as u16);
    dev.get_mmu_mut().write_u8(address, value);
}

/// Loads a value from the device memory at the address (0xff00 + u8) into a 8bit register.
fn ldh_r8_u8(dev: &mut EmulatorDevice, dst: RegisterR8) {
    let address_h = dev.cpu.fetch_u8();
    let address   = 0xff00 | (address_h as u16);
...
fn ldh_r8ptr_r8(dev: &mut EmulatorDevice, dst_ptr: RegisterR8, src: RegisterR8) {
...
fn ldh_r8_r8ptr(dev: &mut EmulatorDevice, dst: RegisterR8, src_ptr: RegisterR8) {
...
fn push_r16(dev: &mut EmulatorDevice, r16: RegisterR16) {
    let value = dev.cpu.get_r16(r16);
    dev.cpu.push_u16(value);
}

/// Pops a 16bit value from the stack into a 16bit register.
fn pop_r16(dev: &mut EmulatorDevice, r16: RegisterR16) {
    let value = dev.cpu.pop_u16();
    dev.cpu.set_r16(r16, value);
}

/// Pops a 16bit value from the stack into a 16bit register.
/// Applies a bitmask to the value before writing into the register.
fn pop_r16_mask(dev: &mut EmulatorDevice, r16: RegisterR16, mask: u16) {
    let value = dev.cpu.pop_u16();
...
// LD r8, u8
opcode!(ld_a_u8, [dev] ld_r8_u8(dev, RegisterR8::A));
opcode!(ld_b_u8, [dev] ld_r8_u8(dev, RegisterR8::B));
opcode!(ld_c_u8, [dev] ld_r8_u8(dev, RegisterR8::C));
opcode!(ld_d_u8, [dev] ld_r8_u8(dev, RegisterR8::D));
opcode!(ld_e_u8, [dev] ld_r8_u8(dev, RegisterR8::E));
opcode!(ld_h_u8, [dev] ld_r8_u8(dev, RegisterR8::H));
opcode!(ld_l_u8, [dev] ld_r8_u8(dev, RegisterR8::L));

// LD r16, u16
opcode!(ld_bc_u16, [dev] ld_r16_u16(dev, RegisterR16::BC));
opcode!(ld_de_u16, [dev] ld_r16_u16(dev, RegisterR16::DE));
opcode!(ld_hl_u16, [dev] ld_r16_u16(dev, RegisterR16::HL));

// LD r8, (u16)
opcode!(ld_a_u16ptr, [dev] ld_r8_u16ptr(dev, RegisterR8::A));

// LD r8, (r16)
opcode!(ld_a_bcptr, [dev] ld_r8_r16ptr(dev, RegisterR8::A, RegisterR16::BC));
opcode!(ld_a_deptr, [dev] ld_r8_r16ptr(dev, RegisterR8::A, RegisterR16::DE));
opcode!(ld_a_hlptr, [dev] ld_r8_r16ptr(dev, RegisterR8::A, RegisterR16::HL));
opcode!(ld_b_hlptr, [dev] ld_r8_r16ptr(dev, RegisterR8::B, RegisterR16::HL));
opcode!(ld_c_hlptr, [dev] ld_r8_r16ptr(dev, RegisterR8::C, RegisterR16::HL));
opcode!(ld_d_hlptr, [dev] ld_r8_r16ptr(dev, RegisterR8::D, RegisterR16::HL));
opcode!(ld_e_hlptr, [dev] ld_r8_r16ptr(dev, RegisterR8::E, RegisterR16::HL));
opcode!(ld_h_hlptr, [dev] ld_r8_r16ptr(dev, RegisterR8::H, RegisterR16::HL));
opcode!(ld_l_hlptr, [dev] ld_r8_r16ptr(dev, RegisterR8::L, RegisterR16::HL));

// LD (r16), r8
opcode!(ld_bcptr_a, [dev] ld_r16ptr_r8(dev, RegisterR16::BC, RegisterR8::A));
opcode!(ld_deptr_a, [dev] ld_r16ptr_r8(dev, RegisterR16::DE, RegisterR8::A));
opcode!(ld_hlptr_a, [dev] ld_r16ptr_r8(dev, RegisterR16::HL, RegisterR8::A));
opcode!(ld_hlptr_b, [dev] ld_r16ptr_r8(dev, RegisterR16::HL, RegisterR8::B));
opcode!(ld_hlptr_c, [dev] ld_r16ptr_r8(dev, RegisterR16::HL, RegisterR8::C));
opcode!(ld_hlptr_d, [dev] ld_r16ptr_r8(dev, RegisterR16::HL, RegisterR8::D));
opcode!(ld_hlptr_e, [dev] ld_r16ptr_r8(dev, RegisterR16::HL, RegisterR8::E));
opcode!(ld_hlptr_h, [dev] ld_r16ptr_r8(dev, RegisterR16::HL, RegisterR8::H));
opcode!(ld_hlptr_l, [dev] ld_r16ptr_r8(dev, RegisterR16::HL, RegisterR8::L));

// LD A, (HL+)
opcode!(ld_a_hlptri, [dev] {
    let hl = dev.cpu.get_r16(RegisterR16::HL);
    ld_r8_addr(dev, RegisterR8::A, hl);
    dev.cpu.set_r16(RegisterR16::HL, hl + 1);
});

// LD A, (HL-)
opcode!(ld_a_hlptrd, [dev] {
    let hl = dev.cpu.get_r16(RegisterR16::HL);
    ld_r8_addr(dev, RegisterR8::A, hl);
    dev.cpu.set_r16(RegisterR16::HL, hl - 1);
});
...
// LD (HL+), A
opcode!(ld_hlptri_a, [dev] {
    let hl = dev.cpu.get_r16(RegisterR16::HL);
    ld_addr_r8(dev, hl, RegisterR8::A);
    dev.cpu.set_r16(RegisterR16::HL, hl + 1);
});

// LD (HL-), A
opcode!(ld_hlptrd_a, [dev] {
    let hl = dev.cpu.get_r16(RegisterR16::HL);
    ld_addr_r8(dev, hl, RegisterR8::A);
    dev.cpu.set_r16(RegisterR16::HL, hl - 1);
});
...
// LD (r16), u8
opcode!(ld_hlptr_u8,  [dev] ld_r16ptr_u8(dev, RegisterR16::HL));

// LD (u16), A
opcode!(ld_u16ptr_a,  [dev] ld_u16ptr_r8(dev, RegisterR8::A));

// LD (u16), SP
opcode!(ld_u16ptr_sp, [dev] ld_u16ptr_u16v(dev, dev.cpu.get_stack_pointer()));

// LDH (0xff00 + u8), A // LDH A, (0xff00 + u8)
opcode!(ldh_u8_a, [dev] ldh_u8_r8(dev, RegisterR8::A));
opcode!(ldh_a_u8, [dev] ldh_r8_u8(dev, RegisterR8::A));

// LDH (0xff00 + C), A // LDH A, (0xff00 + C)
opcode!(ldh_cptr_a, [dev] ldh_r8ptr_r8(dev, RegisterR8::C, RegisterR8::A));
opcode!(ldh_a_cptr, [dev] ldh_r8_r8ptr(dev, RegisterR8::A, RegisterR8::C));

// LD HL, SP+i8
opcode!(ld_hl_sp_i8, [dev] {
...
// PUSH r16
opcode!(push_af, [dev] push_r16(dev, RegisterR16::AF));
opcode!(push_bc, [dev] push_r16(dev, RegisterR16::BC));
opcode!(push_de, [dev] push_r16(dev, RegisterR16::DE));
opcode!(push_hl, [dev] push_r16(dev, RegisterR16::HL));

// POP r16
opcode!(pop_af, [dev] pop_r16_mask(dev, RegisterR16::AF, 0xfff0));
opcode!(pop_bc, [dev] pop_r16(dev, RegisterR16::BC));
opcode!(pop_de, [dev] pop_r16(dev, RegisterR16::DE));
opcode!(pop_hl, [dev] pop_r16(dev, RegisterR16::HL));
