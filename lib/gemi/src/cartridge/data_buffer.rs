...
use crate::core::cartridge::image_data::{ImageData, ImageDataMut};
use crate::core::utils::{ioerr, SerializableBuffer};
...
/// Represents a data buffer that holds binary data of arbitrary size.
///
/// The data buffer can be initialized with a byte data.
///
/// If supported, the data buffer can be written to and read from a file.
/// This requires the `file_io` feature to be enabled.
#[derive(Clone)]
...
impl DataBuffer {
    ...
    /// Copies the data from a byte array slice into the current buffer.
    /// Fails if the size of the byte array is not equal to the size of the RAM data.
    pub fn read_from_bytes(&mut self, bytes: &[u8]) -> Result<(), ioerr::ErrorCode> {
        ...
        self.get_data_mut().copy_from_slice(bytes);
